...
class ICal {
    ...
    /**
     * Returns an array of arrays with all events in a condensed format. Every event is an associative
     * array and each property is an element it.
     *
     * @return {array}
     */
    public function eventsCondensed($color = "white") {
        $events = $this->events();
        $condensedEvents = [];
        ...
        foreach ($events as $event) {
            $condensedEvents[] = [
                'title' => $event['SUMMARY'],
                'start' => date("Y-m-d H:i:s", $this->iCalDateToUnixTimestamp($event['DTSTART'])),
                'end' => date("Y-m-d H:i:s", $this->iCalDateToUnixTimestamp($event['DTEND'])),
                'color' => $color,
                "textColor" => "black",
                "editable" => false,
